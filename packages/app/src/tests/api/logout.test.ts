...
describe('Logout', () => {
  suite('Success', () => {
    ...
    test('Logout', async () => {
      ...
      expect(response.body.data.logoutUser).toEqual('OK');

      const setAuthTokenCookie = response.headers['set-cookie'][0];
      expect(
        setAuthTokenCookie.includes('auth_token=; Max-Age=0;')
      ).toBeTruthy();

      const setRefreshTokenCookie = response.headers['set-cookie'][1];
      expect(
        setRefreshTokenCookie.includes('refresh_token=; Max-Age=0;')
      ).toBeTruthy();
    });
  });
});
